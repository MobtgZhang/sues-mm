...
if %flag%x == thesisx (
	...
	if ERRORLEVEL 1 (
		echo.
		echo %ESC%[31mError! Please check the %ESC%[7m'%THESIS%.log,%COMMITMENT%.log,%FORMAT%.log'%ESC%[0;31m for more details . . .%ESC%[0m
		pause
	) else (
		call :clean
...
:help
	echo %ESC%[33mThis is the compile batch script for SUES MM.
	echo Usage:
...
:thesis
    ...
    latexmk -pdfxe -quiet -file-line-error -halt-on-error -interaction=nonstopmode %COMMITMENT% 2>nul
    latexmk -pdfxe -quiet -file-line-error -halt-on-error -interaction=nonstopmode %FORMAT% 2>nul
goto :EOF

:clean
	echo %ESC%[33mClean files . . .%ESC%[0m
	latexmk -pdfxe -quiet -c %THESIS% 2>nul
    latexmk -pdfxe -quiet -c %COMMITMENT% 2>nul
    latexmk -pdfxe -quiet -c %FORMAT% 2>nul
goto :EOF
...
:cleanall
    ...
    if exist %COMMITMENT%.pdf (
		echo %ESC%[31mClose the file: %ESC%[7m'%COMMITMENT%.pdf'%ESC%[0;31m!%ESC%[0m
		pause
		call :cleanall
	)
    if exist %FORMAT%.pdf (
		echo %ESC%[31mClose the file: %ESC%[7m'%FORMAT%.pdf'%ESC%[0;31m!%ESC%[0m
		pause
		call :cleanall
	)
goto :EOF
